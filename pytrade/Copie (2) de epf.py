...
def cid(v):
  format_cours_intraday (lire_isin(v))

def epf(intraday,t1,periode,n,tau0,tau1):
  master=Tk()
  ...
  for o in range (n):
    ...
    for j in range(1,len(intraday)-1):
      ...
      if t <= t1:
        ...
        phi1 = phi1 + c * math.cos (t / tau) * (ts-tp)/2
        psi1 = psi1 + c * math.sin (t / tau) * (ts-tp)/2 
  ...
  for t in range(imin,imax):
    ...
    for o in range (n):
      ...
      d = d + 1.0/(tau*tau) * (phi[o] * math.cos(t/tau) - psi[o] * math.sin(t/tau))
      # print 'tau=', tau, ' d=', d
    # d = d / n 
