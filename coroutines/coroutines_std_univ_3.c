...
int coroutine_actions (coroutine me, int intro)
{
...
int maincor_actions (coroutine me, int intro)
{
...
int *stack;
	...
	cr->caller = &my_env;
	cr->called = &your_env;
	answer = (char *) start (cr, maincor_actions, (int)"It's up to you, Maincor !", stack+STACK_SIZE);
	for (;;)
	{
