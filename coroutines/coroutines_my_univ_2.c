...
int maincor_actions (jmp_buf *mainprog, jmp_buf *myself, int intro)
{
jmp_buf me[1], you[1];
char *answer;
//#define STACK_SIZE 1000
...
int *stack;
	stack = (int *) calloc (STACK_SIZE+1, sizeof(int));
	answer = (char *) start (me, you, maincor_actions, (int)"Hello Maincor, it's up to you !", stack+STACK_SIZE);
	printf ("Main got %s from Maincor\n", answer);
	answer = (char *) cont (me, you, (int)"Good luck !");
	printf ("Then Main got %s from Maincor\n", answer);
}
