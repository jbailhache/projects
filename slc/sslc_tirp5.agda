...
-- variable substitution
 ...
 abstr : Proof -> String -> LambdaTerm -> Proof
 abst : Proof -> String -> LambdaTerm -> Proof
 ...
 abstr d v (var u) = if v eq u then d else SMB
 abstr d v (lbd u x) = abst d u (abst d v x)
 ...
 ProofOfLambdaTerm (dbl x y) = DBL (ProofOfLambdaTerm x) (ProofOfLambdaTerm y)
 ProofOfLambdaTerm (var s) = SMB
 ProofOfLambdaTerm (lbd s x) = abst DB0 s x
