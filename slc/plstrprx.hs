module PL where
 ...
 shift u (XOP x y z) = XOP (shift u x) (shift u y) (shift u z)
 ...
 subst1 u (XOP x y z) b = XOP (subst u x b) (subst u y b) (subst u z b)
 ...
 cont1 (XOP x y z) t = (cont x t) || (cont y t) || (cont z t)
 ...
 red1 (XOP (SMB "+") x y) = 
  case red x of
   SMB x1 -> case red y of
              SMB y1 -> SMB (show ((read x1) + (read y1)))
              _ -> XOP (SMB "+") (red1 x) (red1 y)
   _ -> XOP (SMB "+") (red1 x) (red1 y)
 red1 (XOP x y z) = XOP (red1 x) (red1 y) (red1 z)
 ...
 -- side RightSide (XOP (SMB "+") (SMB y) (SMB z)) = SMB (show ((read y) + (read z)))
 side s (XOP x y z) = XOP (side s x) (side s y) (side s z)
 ...
 abstr1 d v (XOP x y z) = XOP (abstr d v x) (abstr d v y) (abstr d v z)
 ...
 showproof1 _ col (RED x) = "@" ++ showproof1 Other (col+1) x
 ...
 showproof1 _ col (XOP x y z) = let s = showproof1 Other (col+1) x in "&" ++ s ++ " " ++ let t = showproof1 Other (column (col+1) s + 1) y in t ++ " " ++ showproof1 Other (column (column (col+1) s + 1) t + 1) z
 ...
 pl1 ('&' : s) = let (x, t) = pl1 s in let (y, u) = pl1 t in let (z, v) = pl1 u in (XOP x y z, v)
