...
struct proof1 {
	...
	proof sp1, sp2, red, side[2], val, cond;	
};
...
proof pair (proof x, proof y) {
	proof z;
	z = fnc(apl(apl(var,x),y));
	/*
	printf("\npair x : ");
...
proof assuming (proof x, proof y) {
	proof z;
	if (y->cond == NULL) {
		z = x;
	} else {
	...
	check_memory(); 
	proofs[nproofs].op = y->op; 
	proofs[nproofs].cert = y->cert;
	strcpy(proofs[nproofs].name,y->name);
	proofs[nproofs].sp1 = y->sp1; 
	proofs[nproofs].sp2 = y->sp2; 
	proofs[nproofs].val = y->val;
	proofs[nproofs].cond = z;
	/*
	printf("\nassuming x : ");
	print_proof_to_stdout(x);
	printf("\n         y : ");
	print_proof_to_stdout(y);
	printf("\nresult     : ");
	print_proof_to_stdout(proofs+nproofs);
	*/
	return proofs+nproofs++; 	
}
...
proof and_cond (proof x, proof y) {
	if (x == NULL || x == true_cond) {
		return y;
	}
	if (y == NULL || y == true_cond) {
		return x;
	}
	return pair(x,y);
}
...
proof cond (proof x) {
	...
	switch (x->op) {
		...
		case GTR :
		case EQU :
			return and_cond(x->sp1->cond, x->sp2->cond);
		case PRT :
			return x->sp2->cond;
		default :
			return true_cond;
	...
	for (;;) {
		...
		switch(cur_char) {
			...
			case '|' :
				nextchar(reader);
				// printf("\n(1) u : ");
				// print_proof_to_stdout(u);
				u = sass(u,sgtr(t,sequ(x,y)));
				// printf("\n(2) u : ");
				// print_proof_to_stdout(u);
				x = NULL;
				y = NULL;
...
void print_proof_1(struct printer *printer, proof x, int parenthesized, int full) {
	...
	x->op = SMB;
	/*if (x->cond != NULL) {
		print_proof_1(printer, x->cond, 0, 0);
	...
	if (filename) {
		...
		for (;;) {
			...
			if (!quiet) {
				...
				if (c != NULL && c != true_cond) {
					printf("\nassuming   : ");
					print_proof_to_stdout(c);
				}
				printf("\n");
			}
		}
