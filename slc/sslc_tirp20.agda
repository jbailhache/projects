...
-}
 ...
 LT_rpt = apl LT_Y (lambda "rpt" (lambda "n" (lambda "f" (lambda "x" 
  (apl (apl (var "n")
   (var "x") )
   (lambda "p" (apl (apl (apl (var "rpt") (var "p")) (var "f")) (apl (var "f") (var "x")))) ) ))))

 -- w = Lim (\n -> rpt n Suc Zero)

 LT_w = apl LT_Lim (lambda "n" (apl (apl (apl LT_rpt (var "n")) LT_Suc) LT_Zero))
