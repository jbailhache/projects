...
-}
 ...
 data Rule2 = EQU | APL | LTR | LTRL | LTN | LT2 | LTS | TRN | SUB | LBD
  deriving (Eq)
...
-}
 ...
 slc1 ("LTRL" : s) e = let (x, t) = slc1 s e in let (y, u) = slc1 t e in (ltrl x y, u)
...
-}
 ...
 red1b (Proof1 NRD x) = (x, True)
 red1b (Proof2 APL (Proof0 (FNC f)) y) = (f y, False)
 red1b (Proof2 APL (Proof1 DBL x) y) = (subst (Proof0 DB0) x y, False)
 red1b (Proof2 APL (Proof2 LBD v x) y) = (subst v x y, False)
 red1b (Proof1 DBP (Proof1 DBS x)) = (x, False)
 red1b (Proof1 DBP (Proof1 DBL x)) = (Proof1 DBL (Proof1 DBP x), False)
 red1b (Proof0 r) = (Proof0 r, False)
 red1b (Proof1 r x) = let (y, b) = red1b x in (Proof1 r y, b)
 red1b (Proof2 r x y) =
  let (x1, b) = red1b x in 
   if x == x1 
   then 
    let (y1, c) = red1b y in 
     (Proof2 r x y1, c)
   else (Proof2 r x1 y, b)

 nreduce (Proof1 NRD x) = x
 nreduce x = let (y, b) = red1b x in 
  if y == x 
  then x 
 ...
 side s u v (Proof2 LTR x y) =
    ...
    -- then reduce (side RightSide u v (if s == LeftSide then x else y)) -- initial definition
    then (side RightSide u v (if s == LeftSide then x else y)) -- variant 
    else Proof2 LTR x y
 -- LTRL : a = b, c = d |- if reduce(a) == reduce(c) then b = d  
 side s u v (Proof2 LTRL x y) =
    if reduce (side LeftSide u v x) == reduce (side LeftSide u v y) 
    then side RightSide u v (if s == LeftSide then x else y)
    else Proof2 LTRL x y
 ...
 -- STL : a = b |- redstep(a) = b
 side LeftSide u v (Proof1 STL x) = redstep (side LeftSide u v x)
 side RightSide u v (Proof1 STL x) = side RightSide u v x
 -- STR : a = b |- a = redstep(b)
 side LeftSide u v (Proof1 STR x) = side LeftSide u v x
 side RightSide u v (Proof1 STR x) = redstep (side RightSide u v x)
 -- ST2 : a = b |- redstep(a) = redstep(b)
 side s u v (Proof1 ST2 x) = redstep (side s u v x)
 -- SLQ : x |- redstep(x) = x
 -- SLQ x = STL (QOT x)
 side LeftSide u v (Proof1 SLQ x) = redstep x
 side RightSide u v (Proof1 SLQ x) = x
 -- SRQ : x |- x = redstep(x)
 -- SRQ x = STR (QOT x)
 ...
 side RightSide u v (Proof1 RDL x) = side RightSide u v x
 -- RED : a = b |- a = reduce(b)
 ...
 -- RLQ : x |- reduce(x) = x
 -- RLQ x = RDL (QOT x)
 side LeftSide u v (Proof1 RLQ x) = reduce x
 side RightSide u v (Proof1 RLQ x) = x
 -- RDQ : x |- x = reduce(x)
 -- RDQ x = RED (QOT x)
...
-}
 ...
 gp x y = apl2 (smb "gp") x y
 pt x y = apl2 (smb "pr") x y

 gpr1 = lambda "x" $ lambda "y" $ lambda "z" $ 
  equ (apl (pt (var "x") (var "y")) $
       apl (pt (var "y") (var "z")) $
       gp (var "x") (var "z"))
      ident
 gpa1 = equ (pt a b) ident
 gpa2 = equ (pt b c) ident
 
 gpt1 = ltr (gp a c) (ltr (apl gpa2 (gp a c)) (ltr (ltr (ltr (apl gpa1 (apl (pt b c) (gp a c))) (apl3 gpr1 a b c)) (apl (pt b c) (gp a c))) (dbl db0)))
 
 apr r x = ltr (ltr (apl r x) (reduce (left (apl r x)))) (reduce (right (apl r x)))
 
 -- apr3 r x y z = ltr (ltr (apl3 r x y z) (reduce (left (apl3 r x y z)))) (reduce (right (apl3 r x y z)))
 -- proves (apr3 gpr1 a b c)

 apr2 r x y = apr (apr r x) y

 apr3 r x y z = apr (apr (apr r x) y) z
 
-- Slcr> proves (ltr (apr gpa2 (gp a c)) (ltr (apr gpa1 (apr (pt b c) (gp a c))) (apr3 gpr1 a b c)))

-- The proof
-- LTR LTR LTR (= (pr b c) [*] : gp a c) (pr b c : gp a c) (gp a c) LTR LTR LTR (= (pr a b) [*] LTR LTR (pr b c : gp a c) (pr b c : gp a c) (pr b c : gp a c)) (pr a b : pr b c : gp a c) (pr b c : gp a c) LTR LTR (LTR LTR (LTR LTR ([[[= (pr ''* '* : pr '* * : gp ''* *) [*]]]] a) [[pr a '* : pr '* * : gp a *]] [[[*]]] b) [pr a b : pr b * : gp a *] [[*]] c) (pr a b : pr b c : gp a c) [*]
-- proves
-- (gp a c)
-- equals
-- [*]
 ...
 -- gpLemma6d = ltr gpLemma4d gpLemma4d
 -- gpTheorem1d = ltr gpLemma6d gpLemma5d
