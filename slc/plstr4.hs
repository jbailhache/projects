module PL where
 ...
 pl1 ('[' : s) = let (x, t) = pl3 ']' (FNC VAR) s in (FNC x, t)
 pl1 ('-' : s) = let (x, t) = pl1 s in let (y, u) = pl1 t in (APL x y, u)
 pl1 ('%' : s) = let (x, t) = pl1 s in let (y, u) = pl1 t in (LTR x y, u)
 pl1 ('{' : s) = let (x, t) = pl3 ',' (FNC VAR) s in let (y, u) = pl3 '}' (FNC VAR) t in (LTR x y, u)
 ...
 pl2 e ('\n' : s) = pl2 e s
 pl2 e "" = (Nothing, "")
 pl2 e (c : s) = if c == e then (Nothing, s) else let (x, t) = pl1 (c : s) in (Just x, t)
 
 pl3 e x s = case pl2 e s of
	(Nothing, t) -> (x, t)
	(Just y, t) -> case x of 
	                   FNC VAR -> pl3 e y t
	                   x1 -> pl3 e (APL x1 y) t
