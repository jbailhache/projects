...
-- SLC terms are proofs
 ...
 data Proof : Set where
  ...
  LFT : Proof -> Proof 
  RGT : Proof -> Proof
  LFX : Proof -> Proof -> Nat -> Proof -> Proof
  RGX : Proof -> Proof -> Nat -> Proof -> Proof
  AXM : Proof
  AXN : Nat -> Proof
  RUL : String -> Proof
 ...
 data ProvedEqX : Set where
  having_equals_proof_proves_equals_ : (Nat -> Proof) -> (Nat -> Proof) -> Proof -> Proof -> Proof -> ProvedEqX
...
-- EQU x y === EQU x' y' = (x === x') and (y === y')
 ...
 LFT x === LFT x' = x === x'
 RGT x === RGT x' = x === x'
 LFX a b n x === LFX a' b' n' x' = (((a === a') and (b === b')) and (n == n')) and (x === x')
 RGX a b n x === RGX a' b' n' x' = (((a === a') and (b === b')) and (n == n')) and (x === x')
 AXM === AXM = true
 AXN n === AXN p = n == p
 RUL r === RUL s = r eq s
 x === y = false
 ...
 shift m (LFX a b n x) = LFX (shift m a) (shift m b) n (shift m x)
 shift m (RGX a b n x) = RGX (shift m a) (shift m b) n (shift m x)
 shift m AXM = AXM
 shift m (AXN n) = AXN n
 shift m (RUL r) = (RUL r)
...
-- variable substitution
 ...
 subst n (LFX a b p x) z = LFX (subst n a z) (subst n b z) p (subst n x z)
 subst n (RGX a b p x) z = RGX (subst n a z) (subst n b z) p (subst n x z)
 subst n AXM z = AXM
...
-- reduction
 red1 : Proof -> Proof
 red1 (APL (APL (APL (APL (APL (APL (APL (APL (APL (APL (SMB "quote") (SMB n)) s) v) l) a) t) r) g) d) e) = APL s (SMB n)
 red1 (APL (APL (APL (APL (APL (APL (APL (APL (APL (APL (SMB "quote") (DBV p)) s) v) l) a) t) r) g) d) e) = APL v (proofOfNat p)
 red1 (APL (APL (APL (APL (APL (APL (APL (APL (APL (APL (SMB "quote") (DBL x)) s) v) l) a) t) r) g) d) e) = APL l (quot x)
 red1 (APL (APL (APL (APL (APL (APL (APL (APL (APL (APL (SMB "quote") (APL x y)) s) v) l) a) t) r) g) d) e) = APL (APL a (quot x)) (quot y)
 red1 (APL (APL (APL (APL (APL (APL (APL (APL (APL (APL (SMB "quote") (LTS x y)) s) v) l) a) t) r) g) d) e) = APL (APL t (quot x)) (quot y)
 red1 (APL (APL (APL (APL (APL (APL (APL (APL (APL (APL (SMB "quote") (RED x)) s) v) l) a) t) r) g) d) e) = APL r (quot x)
...
-- red1 (APL (APL (APL (APL (APL (APL (APL (APL (APL (APL (SMB "quote") (RUL n)) s) v) l) a) t) r) g) d) e) = APL e (SMB n)
 red1 (APL (APL (APL (APL (APL (APL (APL (APL (APL (APL (SMB "quote") (LFX a' b' n x)) s) v) l) a) t) r) g) d) e) = APL (APL (APL (APL g (quot a')) (quot b')) (quot (proofOfNat n))) (quot x)
 red1 (APL (APL (APL (APL (APL (APL (APL (APL (APL (APL (SMB "quote") (RGX a' b' n x)) s) v) l) a) t) r) g) d) e) = APL (APL (APL (APL d (quot a')) (quot b')) (quot (proofOfNat n))) (quot x)
 red1 (APL (APL (APL (APL (APL (APL (APL (APL (APL (APL (SMB "quote") (AXN n)) s) v) l) a) t) r) g) d) e) = APL e (proofOfNat n)
 red1 (SMB x) = (SMB x)
 red1 (DBV n) = DBV n
 ...
 red1 (LFX a b n x) = LFX (red1 a) (red1 b) n (red1 x)
 red1 (RGX a b n x) = RGX (red1 a) (red1 b) n (red1 x) 
 red1 AXM = AXM
 red1 (AXN n) = AXN n
 red1 (RUL r) = RUL r
...
-- does a proof contain a symbol ?
 ...
 (LFX a b n x) contSym s = ((a contSym s) or (b contSym s)) or (x contSym s)
 (RGX a b n x) contSym s = ((a contSym s) or (b contSym s)) or (x contSym s)
 AXM contSym s = false
...
-- abstraction
 ...
 abstr n s (LFX a b p x) = LFX (abst n s a) (abst n s b) p (abst n s x)
 abstr n s (RGX a b p x) = RGX (abst n s a) (abst n s b) p (abst n s x)
 abstr n s AXM = AXM
...
-- quot x = APL (SMB "quote") x
 ...
 leftx : Proof -> Proof -> Proof -> Proof -> Proof
 leftx a b n x = APL (APL (APL (APL (SMB "leftx") a) b) n) x

 rightx : Proof -> Proof -> Proof -> Proof -> Proof
 rightx a b n x = APL (APL (APL (APL (SMB "rightx") a) b) n) x
...
-- or sidex s a x whit axioms a i true = a i false
 ...
 sidex : Bool -> (Nat -> Proof) -> (Nat -> Proof) -> Proof -> Proof
 sidex _ _ _ (SMB n) = (SMB n)
 sidex _ _ _ (DBV n) = (DBV n)
 sidex s a b (DBL x) = DBL (sidex s a b x)
 sidex s a b (APL x y) = APL (sidex s a b x) (sidex s a b y)
...
-- side s (LFT x) = if s then left (quot x) else quot (side true x)
------ sidex s a b (LFX a' b' x) = if s then leftx (quot a') (quot b') (quot x) else quot (sidex true a' b' x)
------ sidex s a b (LFX a' b' n x) = if s then leftx (quot a') (quot b') (quot (proofOfNat n)) (quot x) else quot (sidex true (a' n) (b' n) x)
 -- side s (LFT x) = if s then left (quot (side s x)) else quot (side true (side s x))
------ sidex s a b (RGX a' b' x) = if s then rightx (quot a') (quot b') (quot x) else quot (sidex false a' b' x)
 -- side s (RGT x) = if s then right (quot (side s x)) else quot (side false (side s x))
 -- sidex s a b AXM = if s then a else b
 sidex s a b (AXN n) = if s then a n else b n
 sidex s _ _ (RUL "evq") = lambda "X" (if s then (eval (quot X)) else X)
 sidex s _ _ (RUL "equal") = lambda "X" (if s then (eql X X) else T)
-- sidex s _ _ (RUL "refl") = lambda "X" (lambda "Y" (lambda "Z" (if s then (eql (left X) Y) |- ((eql (right X) Z) |- eql Y Z) else T)))
{-
...
-- representation of proofs by strings
 ...
 rep (LFX a b n x) = "(LFX " cat rep a cat " " cat rep b cat " " cat (repNat n) cat rep x cat ")"
 rep (RGX a b n x) = "(RGX " cat rep a cat " " cat rep b cat " " cat (repNat n) cat rep x cat ")"
 rep AXM = "AXM"
...
-- "(LTS (RED (axiom2 ((gparent Allan) Charles))) (LTS (RED (axiom1 (((parent Brenda) Charles) ((gparent Allan) Charles)))) (RED (((gparent Allan) Brenda) Charles)))) |- ((gparent Allan) Charles) = [$O]"
 ...
 developx : (Nat -> Proof) -> (Nat -> Proof) -> Proof -> ProvedEqX
