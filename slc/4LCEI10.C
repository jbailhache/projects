...
enum node {
	...
	node_subst, node_apsubst, node_apsubstr, node_substlr, node_axiom, node_read,
...
#define apsubstr(f,a) mkdem (node_apsubstr, 0, NULL, f, a)
...
int i;
	...
	{
		if ((name != NULL) && (pm->dems[i]._name != NULL) && !strcmp(name,pm->dems[i]._name))
			return pm->dems+i;
	...
	/* pm->dems[pm->ndems]._exec = pm->dems+pm->ndems; */
	pm->dems[pm->ndems]._exec = NULL;
...
dem r0, r1, r;
	...
	return ap (subst (n, sd0(y), z),
		   subst (n, sd1(y), z));
    if (node(y) == node_apsubstr)
        return apsubstr (subst (n, sd0(y), z),
                         subst (n, sd1(y), z));
...
dem DBname (int n, dem x, dem y)
{
	...
	return ap (DBname (n, x, sd0(y)),
		   DBname (n, x, sd1(y)));
    if (node(y) == node_apsubstr)
        return apsubstr (DBname (n, x, sd0(y)),
                         DBname (n, x, sd1(y)));
    if (node(y) == node_lambda)
...
dem d1, f, a, s0, s1, ld1;
	...
	switch (node(d))
	{
		...
		case node_apsubstr :
			...
			print (sd1(d));
			*/
			if (sd0(d)->_name && !strcmp (sd0(d)->_name, "*sym"))
			{
				return lr (1-r, sd1(d));
			}
			else if (sd0(d)->_name && !strcmp (sd0(d)->_name, "*<"))
			{
				return lr (0, sd1(d));
			}
			else if (sd0(d)->_name && !strcmp (sd0(d)->_name, "*>"))
			...
			else if (node(sd0(d))==node_apsubst && sd0(sd0(d))->_name && !strcmp(sd0(sd0(d))->_name,"*/<"))
			{
			 if (lr(0,sd1(sd0(d)))==lr(0,sd1(d)))
			  /* return lr(1,d->_subdem[r]); */
			  if (r==0)
			   return lr(1,sd1(sd0(d)));
			  else
			   return lr(1,sd1(d));
			 else
			  return d;
			...
			*/
			...
			if (r==1 && node(f)==node_lambda)
			 d1=subst(0,sd0(f),a);
			/*
                        else if (node(d) == node_apsubst && r==0 && node(lr(0,sd0(d)))==node_lambda)
			 d1=subst(0,sd0(lr(0,sd0(d))),lr(0,sd1(d)));
			*/
	...
	switch (c)
	{
		...
		case '&':
			...
			switch (c)
			{
				...
				case '$':
					sd0 = _read (pm);
					d = _mkdem (pm, node_substlr, 0, NULL, sd0, NULL);
					break;
