module sslcs_tirp where

{-
 theory with 1 axiom a = b represented by t : Bool -> Proof
 with t true = a and t false = b
 proof AXM
-}
...
-}
 ...
 LT_addrefl = lambda "t" (lambda "s" 
  (apl (apl LT_P (apl (apl LT_refl (var "t")) (var "s"))) (apl (var "t") (var "s"))) )
...
-}
 ...
 addrefl : (Bool -> Proof) -> Bool -> Proof
 addrefl t s = APL (APL (polt LT_P) 
                (APL (APL (polt LT_refl) (APL (APL (polt LT_P) (t true)) (t false))) (polt (if s then LT_K else LT_KI))) )
                (t s) 
...
{-
 tirp : Ord -> (Bool -> Proof) -> Bool -> Proof
 tirp Zero t s = t s
 tirp (Suc x) t s = addrefl (tirp x t) s
 -- tirp (Lim f) t s = polt (apl LT_mapnat (lambda "n" (tirp (f (var "n")) t s)))
 tirp (Lim f) t s = polt (apl LT_mapnat (lambda "n" (apl (apl (apl LT_tirp (rep f)) (rep t)) (rep s))))
-}
