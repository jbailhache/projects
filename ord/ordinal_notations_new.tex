...
\item multiplication : \( a \times 0 = 0 ; a \times suc(b) = (a \times b) + a \)
\item exponentiation : \( a^0 = 1 ; a^{suc(b)} = a^b \times a  \)
\end{itemize}

For natural numbers, the addition and the multiplication are commutative : \( a+b = b+a ; a \times b = b \times a \), but not the exponentiation : generally \( a^b \neq b^a \).
...
Then we can consider the set \( \lbrace 0, 1, 2, 3, \ldots, \omega, \omega+1, \omega+2, \omega+3, \ldots \rbrace \) which is a limit ordinal, and \( \omega, \omega+1, \omega+2, \omega+3, \ldots \) is a fundamental sequence of this ordinal. This ordinal is \( \omega+\omega = \omega \times 2 \) or \( \omega \times 2 \) or \( \omega 2 \).

Then we can go on building greater and greater ordinals : \( \omega \times 3, \ldots, \omega \times \omega = \omega^2, \omega^3, \ldots, \omega^\omega, \omega^{\omega^\omega}, \ldots \).

\bigskip

\( \omega \) is the least ordinal which has not a finite cardinality. We can define a sequence of ordinals \( \omega_k \) where k is a natural number by \( \omega_0 = \omega \) and \( omega_{k+1} \) is the least ordinal whose cardinality is greater than the cardinality of \( \omega_k \).
...
where for any \( \xi \in \beta \) or \( \xi < \beta, f(\xi) \) is an ordinal. A limit ordinal can always be defined as \( lim_{\omega_k} f \) by eventually rearranging the order of elements of \( \beta \). When \( \beta = \omega = \omega_0, lim_\omega f \) will sometimes be written more simply \( lim f \). This is the case for countable limit ordinals, which have the same cardinality as \( \omega \).
...
\item multiplication : \( \alpha \times 0 = 0 ; \alpha \times suc(\beta) = (\alpha \times \beta) + \alpha ; \alpha \times lim_\beta(f) = lim (\xi \mapsto \alpha \times f(\xi)) \)
\item exponentiation : \( \alpha^0 = 1 ; \alpha^{suc(\beta)} = \alpha^\beta \times \alpha ; \alpha^{lim_\beta(f)} = lim_\beta (\xi \mapsto \alpha^{f(\xi)}) \)
\end{itemize}
...
\item \( \psi_1(\alpha)[n] = \psi_1(\alpha[n]) \)
