...
\item \( cof (suc\ \alpha) = 1 \)

\item \( cof (lim_\beta f) = \beta \) if there is no ordinal \( \gamma < \beta \) such that \( lim_\beta f = lim_\gamma g \).
...
\item \( cof\ \omega = \omega \)

\item \( cof (\omega+3) = 1 \)

\item \( cof (\omega \cdot 2) = \omega \)

\item \( cof\ \omega_1 = \omega_1 \)

\item \( cof\ \omega_3 = \omega_3 \)

\item \( cof\ \omega_\omega = \omega \)

\item \( cof\ (\omega_\omega+1) = \omega_\omega+1 \)

\end{itemize}

A regular ordinal is an ordinal which is equal to its cofinality. A singular ordinal is an ordinal which is not regular.
Assuming the axiom of choice, \( \omega_{\alpha+1} \) is regular for any ordinal \( \alpha \). The cofinality of any ordinal is a regular ordinal, which means \( cof(cof\ \alpha) = cof\ \alpha \).
