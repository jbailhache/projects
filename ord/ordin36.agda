...
-}
 ...
 phic r s g b Zero    = H0 (\x -> phi s (\p -> if p == r then b else if p == 1+ r then x else g p)) Zero
 phic r s g b (Suc a) = H0 (\x -> phi s (\p -> if p == r then b else if p == 1+ r then x else g p)) 
                      (Suc (phi s (\p -> if p == s then a else g p)))
 phic r s g b (Lim f) = Lim (\n -> phi s (\p -> if p == s then f n else g p))
