module ordml where
 ...
 rep1 z s O = z
 rep1 z s (SN x) = s (rep1 z s x) 

 rep' : (t : Set) -> t -> (t -> t) -> Nat -> t
 rep' _ z s O = z
 rep' t z s (SN x) = s (rep' t z s x) 
...
-}
 ...
 lim1 : {s : Set} -> (Nat -> s -> Ord) -> s -> Ord
 lim1 f x = Lim (\(p : Nat) -> f p x)

 lim2 : {s t : Set} -> (Nat -> s -> t -> Ord) -> s -> t -> Ord
...
-}
 ...
 H1 : {s : Set} -> ((s -> Ord) -> s -> Ord) -> (s -> Ord) -> s -> Ord 
 H1 f g = lim1 (rep g f)

 H2 : {s t : Set} -> ((s -> t -> Ord) -> s -> t -> Ord) -> (s -> t -> Ord) -> s -> t -> Ord
 H2 f g = lim2 (rep g f)
...
-}
 ...
 gamma : Ord -> Nat -> Nat
 gamma Zero n = O
 gamma (Suc x) n = SN (gamma x n)
 gamma (Lim phi) n = gamma (phi n) n  

 lambda : Ord -> Nat -> Nat
 lambda Zero n = n
 lambda (Suc x) n = lambda x (SN n)
