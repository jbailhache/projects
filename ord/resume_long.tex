...
\begin{center}
\textbf{LES ORDINAUX TRANSFINIS} par Jacques Bailhache, Janvier-mars 2018
\end{center}
...
\section{Ordinaux-arbres (tree ordinals)}

\vspace{-0.4cm}

Un ordinal-arbre a appartient à la classe d'ordinaux-arbres \( \Omega_n (n \in \mathbb{N} \) si on a a = 0, a = a' + 1 pour un ordinal-arbre a' appartenant à la classe d'ordinaux-arbres \( \Omega_n \), ou a est une fonction de  \( \Omega_k \) vers \( \Omega_n \) pour un certain k < n.
...
A tout ordinal-arbre a, on peut associer un ordinal correspondant \( \alpha = |a| \) obtenu en ignorant le choix d'une suite fondamentale particulière, et défini par :
 \( |0| = 0 \) ;
 \( |a+1| = |a|+1 \) ;
 \( |a| = sup |a[b]| \) si a est une fonction de \( \Omega_k \) vers \( \Omega_n \).
...
\section{Fonctions d'écrasement (Ordinal collapsing functions)}
\vspace{-0.4cm}
