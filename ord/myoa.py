...
# March 19, 2012:  Extended the script to (conjectured) full second order arithmetic; exponential space and time reduction in handling of large integers and n-iteration for large n; added several new functions.  This involved rewriting the first half of the script, and extensively modifying class Ordinal.
...
def IsStandard(a, n=0):
	...
	if n == 0:
		...
		return IsBuiltFromBelow(ChangeOmegaN(a[1], m), ChangeOmegaN(a[2], m), m)
	else:
		return IsBuiltFromBelow(a[1], a[2], n)
...
def StandardForm(a, cache=None):
	...
	def MaximizeA(a, b, n, i=0, d=0, ConvertToStandard=True):
		"""Assuming that a and b are in nth standard form, replaces a with the least ordinal a1 such that a1 is n-built from below from b
			i and d and ConvertToStandard are for internal recursion"""
		# We perform an in-order right to left traversal of the term tree of a to find the first counteraxmple (if any) to a being n-built from below from b.
		# The countexample is of the form f = C2(g, h) where f < W_n and g is too large (that is g>=d).  We replace f with W_n and delete everything to the left of f.
		...
		#
		# Including conversion to the nth standard form at the end is necessary; an example is a=CCW0CCW00 and b=CCW00 (prenex form, W stands for W_2).
	...
	def LargeSafeI(a, a1, b):
		"""Assuming that a and b are in nth standard form, find a large value of i such that a1=MaximizeA(a, b, n) equals MaximizeA(a, (i-1, a, b)); may return 0 if i can be arbitrarily large"""
	...
	# The nth standard form of (1, a[1], a[2]) is (1, a1, a[2]) where a1 = MaximizeA(a[1], a[2], n)
...
class Ordinal(object):
	...
	prettyprintargs = {"poweroperator": "**", "useextendedCNF": True, "usephi": False, "usewCK": False, "useC2": True, "Csymbol": "C", "wsymbol": "w", "Wsymbol": "W", "phisymbol": "phi", "wCKsymbol": "wCK", "symbols":"default", "context": None}
...
def C(a, **args):
	"""Return the (notation specific) collapse of a; or collapse of a above base if base keyword argument is supplied, C(a, base=b) = C(Cinv(b)+a). C (without base) is continous, monotonic, and onto.  C is the one variable analog of C2: C2(a_1,C2(a_2,... C2(a_n,base)..)) (if each a_i is maximal) = C(w**a_n+...w**a_2+w**a_1,base=base)."""
	base = args.get("base")
